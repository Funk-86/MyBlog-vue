        ...
        <div class="operations-container">
          ...
          <t-dropdown :min-column-width="125" trigger="click">
            ...
            <t-button class="header-user-btn" theme="default" variant="text">
              ...
              <div class="header-user-account">{{ adminDisplayName }}</div>
              <template #suffix>
                <chevron-down-icon />
              </template>
            </t-button>
...
// SVG 由 vite-plugin-vue2-svg 转为组件，TS 无声明时推断为 string，需断言为组件
...
export default Vue.extend({
  components: {
    MenuContent,
    LogoFull: LogoFull as import('vue').Component,
    Notice,
    Search,
    ViewListIcon,
    LogoGithubIcon,
  ...
  computed: {
    adminDisplayName(): string {
      const basic = (() => {
        try {
          const s = localStorage.getItem('myblog-admin-basic');
          return s ? JSON.parse(s) : {};
        } catch {
          return {};
        }
      })();
      return basic.nickname || this.$store.state.user?.userInfo?.username || '管理员';
    },
    active() {
      if (!this.$route.path) {
        return '';
    ...
    menuCls(): Array<Record<string, boolean>> {
      return [
        {
